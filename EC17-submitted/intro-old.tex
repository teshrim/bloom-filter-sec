...
%(Put this in an appendix for now, until we see how things play
%out.)}
...
First, we note that the security notion forbids~$A$ to win if it outputs $x^* \in \{x_1,x_2,\ldots,x_q\}$, even if one of these was a false positive.  On one hand, this notion is unsatisfying because it would not say that an attacker wins by creating (say) a bogus email address, and then observing that it passes a spam filter that is based on a Bloom filter representation of a whitelist.\footnote{This is one of the motivating examples from NY.}  The attacker is only allowed to win if it finds a false-positive that it has not verified.  On the other hand, the NY notion does seem to capture the intuition that finding additional false positives does not become easier as more are learned.  Our notions will address both sides of this discussion.
%
%\jnote{We discussed this on the phone, but let me put it in writing here. I think this is actually a reasonable choice because it captures the notion that ``even after finding some false positives, it remains hard to find additional false positives.'' Note that in practice finding false positives is ``easy'' since $\epsilon$ might be $1/1000$ rather than negligible.}
%
%\tsnote{For me the question is: is security violated once the first FP is found, or the second? My answer, which is in line with the traditional soundness notion, is that the first FP is a security violation even if finding it isn't ``hard'' by cryptographic standards. But I do appreciate your point.  A natural goal is to say that, in some sense, \emph{adaptivity doesn't make it any easier} to find a FP.  Adapting to what, though? In the traditional notion, $x$~is picked before the coins of~$\mathbf{B}_1$ are flipped, i.e., before the hash functions are sampled.  The direct non-adaptive generalization is to say that $x_1,x_2,\ldots,x_q$ are all picked up front.  The direct \emph{adaptive} generalization would allow $x_1,x_2,\ldots,x_q$ to depend in the coins of~$\mathbf{B}_1$, i.e., the hash functions.  The NY definition captures only indirect dependence: adversaries that can adapt to set-membership queries, but \emph{not} (directly) to the input-output behaviors of the hash functions.  The three notions I've proposed in Figure 1 cover a spectrum of adaptivity: the first is adapting to set-membership queries only, the second to those and hash input-output behavior, the third to set-membership queries and full knowledge of the hash functions.  Each has it's own associated (informal) security interpretation.}
%\tsnote{Perhaps a more obvious way to capture the idea that ``finding some FPs doesn't help find additional ones'' is to parameterize my notion(s) with a number~$f$, and say that that the adversary wins iff it outputs~$f$ FPs (these may have been previously queried to the set-membership oracle). Note that in the ROM (and probably in practice) finding~$f>1$ FPs will require~$f$ times the work.  Right?}
%
%\tsnote{FYI: this is how MAC security against multiple forgeries is defined.  See \texttt{https://eprint.iacr.org/2006/095.pdf} for details.}

Second, we note that the NY syntax does not capture the size of~$M$
as a parameter; \jnote{I think this is also reasonable; typically
security is defined without regard for efficiency} \tsnote{My
complaint isn't about efficiency. It's about the fact that, by not
including $|M|$ in the definition, the trivial scheme I give (just
below) is allowed, and is secure.  Anyway, as I say at the end of
this paragraph, this is a small(er) complaint than what follows}.
\jnote{I would say that the trivial scheme \emph{is} secure!}
\tsnote{Okay, well then we need some way to ``rule out'' the trivial
solution. Perhaps just in the text -- yes, the trivial scheme works,
but it isn't interesting in practice. Also, in the concrete security
bounds that we will prove, the size of the representation is going
to come up, e.g.\ the number of bits in the BF array directly affect
the FP rate. Hence it makes sense to say things like ``an
$(n,k,m)$-filter\ldots'' where $m$ is the size of the
representation.} \jnote{I still disagree: efficiency is orthogonal
to functionality and security.} \fixme{
Let's discuss this verbally, because I feel like we are talking past each other.}The surrounding text does say that
...
%\jnote{FWIW, I think the classical description of Bloom filters has the same problem. I.e., if the hash functions are not fixed, then their description should be counted as part of the length of the data structure. On the other hand, if they are fixed, then the false-positive rate is not well-defined (since there is no probability space to speak of). So in that sense, NY just carried over the problem from the classical literature.}
%
%\tsnote{One can suggest that it \emph{should} be counted in the classical description, but the fact is that it isn't :) And in the 30+ years since the original paper, no one saw fit to correct this? From an algorithmic perspective, I think the classical approach makes sense.  The questions being answered are: how many bits do I need to store a representation of the set for a given FP rate, and how does this compare to the combinatorially computed lower-bound? The description length of the hash functions may be bigger or smaller, but the number of bits you need to hold the set representation is independent of that.  (Imagine that you have long random bitstrings in the set~$\mathcal{S}$.  Then whether your hash functions simply truncate, or they are random oracles, the size of the representation as a function of the target FP rate doesn't change.  Yet the description lengths of the hash functions are drastically different.)}
...
\jnote{Perhaps it is reasonable to consider a definition in which the set is not given to the adversary?}
\tsnote{A FP security notion? Yes, perhaps so.  It's probably a more realistic notion anyway -- is a spammer really going to know my whitelist a priori? (If he does, I probably have bigger problems in my system than receiving new spam email.) Or set of URLs cached by a CDN server?}

Continuing on this point, it seems that the coins of~$\mathbf{B}_1$ (whatever their use) are intended to be kept secret from the adversary.  Thus, to the extent that it addresses real Bloom filters at all, the NY security notion only addresses applications in which the representation is created using secret coins.  But many real-world applications of Bloom filters have no secrets.  NY point to web proxies, which use Bloom filters to compactly represent the objects they store locally, as motivation for their work.  But real web proxies like Squid~\cite{fan2000summary} do \emph{not} use secrets in their Bloom filters.  Squid, in particular, uses the basic Bloom filter with four hash functions, defined as follows: $h_1(x) = \mathrm{MD5}(x)[0,\ldots,31]$, $h_2(x)=\mathrm{MD5}(x)[32,\ldots,63]$, $h_3(x)=\mathrm{MD5}(x)[64,\ldots,95]$, and $h_4(x)=\mathrm{MD5}(x)[96,\ldots,127]$.  Likewise, there are no secrets in the Bloom filters used by Google's BigTable~\cite{chang2008bigtable}, HBase~\cite{xxx}, \tsnote{make an impressive list.}

There are applications in which the proposed Bloom filter instantiations do use secretly keyed hash functions, e.g. $h_j(x) = (f(K_0,x) + j\cdot g(K_1,x))\bmod m$, where $j\in[1,k]$, $m$ is the number of bits in the array, and where $f,g$ are PRFs keyed by secrets $K_0,K_1$. This particular ``encrypted Bloom filter'' instantiation was suggested for use in efficient linking of records across separate medical record databases~\cite{schnell2011novel}.  (It was subsequently shown not to be secure by Niedermeyer et al.~\cite{niedermeyer2014cryptanalysis}, where a clever use of frequency analysis recovered the last names of patients given access only to the Bloom filter array.)
In any case, it is clear that the NY definition does not capture the most widely used applications of Bloom filters, e.g. CDN summary caches, P2P network routing algorithms, etc.
...
We provide various security notions for these primitives... FP with private, public representation coins; set privacy with private, public representation coins; FP/set privacy in the multi-representation setting (outsider attack, think CDNs); FP/set privacy in the multi-representation setting with corruptions (insider attack, think P2P networks) \tsnote{Those are just the ones that come to mind now.  Could also consider authenticity of representations, think CDN or P2P settings.}
...
\begin{itemize}
\item Niedermeyer et al., ``Cryptanalysis of Basic Bloom Filters Used for Privacy-Preserving Record Linkage'', breaking privacy of secret-hash-function Bloom filters. \tsnote{Journal of Privacy and Confidentiality, 2014}
...
%
(I'll also point out that they explicitly say that $B_1$ outputs “compressed representation of [set $S$] $B_1(S)=M$”.  But it would be easy to have small sets $S$ for which the bit-size of the array is smaller than the total bit-size of $S$, yet the combined bit-size of array + hash functions is larger.)
%

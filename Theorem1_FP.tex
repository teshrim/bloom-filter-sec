...
\noindent
Let adversaries $B_1, B_2$ simulate $\game{1}(A)$ and $\game{2}(A)$ respectively as shown in \Cref{fig:BGame1}. When $b=1$, adversaries $B_1$ and $B_2$ exactly simulate $\game{0}(A)$ and $\game{1}(A)$.  When $b=0$, adversary $B_1$ wins if $\game{1}(A)$ outputs 0; adversary $B_2$ wins if $\game{2}(A)$ returns 0.  Thus we have
...
& \leq q\epsilon,\mbox{ where } \epsilon = (1-e^{-kn/m})^k \label{eq:4}
\end{align}

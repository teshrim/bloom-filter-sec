...
\noindent
In $\game{3}(A)$, $t$ elements are sampled from the $q$ adaptive queries and tested for false positives. This is equivalent to $\game{2}(A)$, as the hash functions of the $\HashOracle$ oracle are random and $A$ wins this  game only if it outputs atleast $t$ FP's.\acnote{I was facing problems in connecting game2(adaptive) and game3(non-adaptive). I felt that the probability of  winning both games should be equal as the hash functions in the hash oracle are random(not independent) and adversary in game 2 has no means to output t elements which have higher chance of winning than those in game 3.}
\begin{equation}

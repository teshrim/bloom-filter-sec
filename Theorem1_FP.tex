...
Let~$A$ be an adversary, compatible with $\ExpFPSecHash{B_\mathrm{lin},r}{\cdot}$, that asks at most~$q$ queries in time $t$ to its $\QryOracle$ oracle.  Then 
\[
...
this theorem. Here, $B_1$ and $B_2$, each ask $q$ oracle queries in time O(t). \tsnote{Statement about resources of these reductions.}
\end{theorem}

...
$\calC \gets \calC \cup \{\qry_x\}$\\
$a \gets \Qry^{\HashOracle}(\pubaux,\privaux,\qry_x)$\\
if $a \neq \qry_x(S)$ then \\
\nudge $\err \gets \err + 1$\\
Return~$a$\\
%
...
\oracle{$\TestOracle(\qry_x)$}\\
if $\qry_x \in \mathcal{C}$ then Return $\bot$\\
$\calC \gets \calC \cup \{\qry_x\}$; $M \gets\pubaux$\\
...
\nudge $v_j \getsr [m]$\\
\nudge if $T[j,\qry_y] \neq \undefined$\\
\nudge \nudge $v_j \gets T[j,\qry_y]$\\
\nudge $T[j,\qry_y] \gets v_j$\\

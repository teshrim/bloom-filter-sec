...
output by $\Rep$. \tsnote{How does one define the length of~$M$,
  ...
  bit-length, relative to some implict encoding into bitstrings)?
  Also, why take the max?  Why not just the length of~$M$, whatever it
happens to be?}\tsnote{What is a ``query''?  A predicate? A
real-valued function? A (potentially) randomized algorithm?}
\end{definition}
...
trivially make everything part of~$\pub$. \tsnote{Is the idea that
  ...
  the security defintions, i.e.\ whether we can give~$M$ to the
  attacker or not. } \tsnote{By the way, I'm coming around to the
...
in the syntax.} It captures the fp-bb-pub notion in the
random-oracle model. 
...
enforce it in the experiement, but then our resource accounting risks
overcounting the number of queries.} \tsnote{Also, we need that, in general, $\pub$ itself
does not leak information about~$S_0$ vs.\ $S_1$. }
\end{enumerate}

...
$(\Rep_1, \Rep_2, F)$. If $F$ is a pseudorandom function, then $\Pi$
is semantically secure with respect to leakage function ${\sf leak}(S) = |S|$.
\end{theorem}
\begin{proof}
We define the following simulator $\Sim$: on input $n$, define $S^*=\{x_1, \ldots, x_n\}$
(for distinct elements $x_1, \ldots, x_n$). Then compute
\begin{enumerate}
\item $x_1, \ldots, x_t \getsr \Rep_1(S^*)$,
\item $k \getsr \bool^n$,
\item $\pubaux \getsr \Rep_2(|S|, F_k(x_1), \ldots, F_k(x_t))$,
...
For a given set $S$, define distribution $\mbox{{\sc Real}}_S$ as follows:

...
Switching now to the case of a \emph{private}~$M$ GBF, we have the
following correctness bound.

\todo{Insert correctness theorem for \emph{private}~$M$ in the ROM.}
\begin{theorem}
Fix $k,m,n,r,\lambda>0$, and let $\setprim_{\mathrm{gbf}}=(\Rep, \Qry)$ be the private~$M$ GBF. For any adversary~$A$ that makes a total of~$q_T$ queries to the $\Test$ oracle, and $q_R$ queries to the RO, and has time-complexity~$O(t)$,
\[
...
\todo{Public M, ROM (no, empty priv)}\tsnote{one-wayness suffers the same
  attack as correctness, I think?}

\todo{Public M, PRF hash (follows from meta-theorem)}

\todo{Private M (trivial!)}

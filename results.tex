...
linear-PRF} construction \jnote{Really a bad name\ldots}, was
suggested as a way to allow for privacy-preserving linkage of
medical records~\cite{schnell2011novel}. \jnote{We should really
give more discussion, explaining what the data structure does in
words, and also discussing the goal of ``privacy-preserving record
linkage'' and describing the protocol for doing so (at a high
level).} We only address the privacy of this construction, as the
correctness follows immediately from the correctness of the linear
PRF-hash construction. \jnote{Actually, it seems worth giving a
proof of correctness just to get the parameters right.}

\heading{Privacy. }  Unfortunately, and in contrast to the linear
...
\jnote{Give intuition for why the attack works: the same ``hash
functions'' are used for each ``Bloom filter,'' and this leaks
information about bigrams shared among multiple strings.} \jnote{I
simplified the attack.}
...
\jnote{Could also give all-but-the-2nd bit of $x_n$, but does it
matter?} Define an attacker $A$ as follows: it chooses a uniform
...
In analyzing the behavior of $A$, note first that $\Rep$ begins by
...
this probability is at least~$1/2$.) \jnote{Check my math? Get
tighter analysis?} We conclude that in the real experiment $A$
...
%\begin{theorem}%[{\rm SS-privacy of bigram linear-PRF}]
%\label{thm:bi-lin-ss}
%Let~$\elts=\{\mathrm{a,b,c}\}^*$. Let $n=3$,  $m=3\alpha $ for
%integer $\alpha \gg 1$, and let, $k = \lceil \ln(2) \alpha
%\rceil$.\footnote{This value of~$k$ is from the classical analysis
%for minimizing the FP rate for a Bloom filter, and is set this way
%for compatibility with classical results.  It isn't used in any
...
\caption{Bigram domain-separated PRF-hash. \jnote{There must be some better choice of notation to indicate that
the only change is in the way the hash functions are instantiated. (This is a general comment that
applies to the entire paper.)}}
\label{fig:DS-bigram}
\end{figure}

Niedermeyer et al.~\cite{niedermeyer2014cryptanalysis} suggest
replacing the hash functions in the previous construction with
independently keyed PRFs as a countermeasure to their attack.
\jnote{We don't even say what their attack is.} In terms of
simulation-based privacy, however, it is not hard to see---using a
similar attack as in the previous section---that this is of little
help.
...
\jnote{Fix indentation.}

\jnote{I ran out of steam at this point\ldots}
...
In~\cite{CCS:DonCheWen13} the GBF is a central piece of a private
...
\heading{Correctness.} We note that~\cite{CCS:DonCheWen13} provides
...
Now, this adversary will find a false positive, if there is one, but

...
linkage of medical records~\cite{niedermeyer2014cryptanalysis}. %,schnell2011novel}.
\jnote{We should really give more discussion, explaining what the
data structure does in words, and also discussing the goal of
``privacy-preserving record linkage'' and describing the protocol
for doing so (at a high level).} We only address the privacy of this
construction, as the correctness follows immediately from the
correctness of the linear PRF-hash construction. \jnote{Actually, it
seems worth giving a proof of correctness just to get the parameters
right.}
...
this probability is at least %JK

...
\begin{theorem}
Fix $k,m,n,r,\lambda>0$, a set~$\elts$, and let $\setprim_{\mathrm{gbf}}=(\Rep, \Qry)$ be the garbled Bloom filter (set membership) data structure over~$\elts$. For any adversary~$A$ that makes a total of~$q_T$ queries to the $\Test$ oracle, and $q_R$ queries to the RO, and has time-complexity~$O(t)$,
\[

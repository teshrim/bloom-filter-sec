...
\caption{Correctness attack for garbled Bloom filters with public~$M$. }
\label{fig:adv-gbf}
\end{figure}
...
Consider the adversary described in Figure~\ref{fig:adv-gbf}.  In one
...
set-membership query would be laughable in many applications, and a
$2^{-80}$ false-postive rate would be considered overkill.\footnote{Note
...
be significantly more efficient than brute force guessing over the
universe of inputs (imagine that $\elts=\bits^{128}$), even if you know
that the distribution deviates significantly from uniform. Notice,
too, that you \emph{cannot} mount it against a standard Bloom
filter.
...
j,x \rangle)$ and assume~$F_K$ is a secure PRF.  Here, $\privaux=K$.  Now,
...
\fixme{Insert correctness theorem for \emph{private}~$M$ in the ROM.}
\begin{theorem}
...
\[
\AdvCorrect{\setprim_{\mathrm{gbf}},r}{A} \leq  [...]
\]
\end{theorem}

\heading{Privacy. }
We observe immediately that when~$\pubaux=\langle M \rangle$ the GBF cannot be SS-private (in the ROM) because $\privaux=\emptystring$.  In fact, it is easy to see that the correctness attack just given works as a privacy attack, too, with small modifications.  In particular, the attacker cannot check that the reconstructed $x \not\in S$, nor query a~$\TestOracle$.  Thus the efficacy of the attack depends on the false positive rate. \todo{Either work out a proof for this case, or punt and make the attack more formal.}
...
\begin{theorem}\label{thm:lin-privacy}
Fix $k,m,n,r,\lambda>0$ and a function family~$F\colon \calK \times \bits^* \to [m]$.  Let $\setprim_{\mathrm{gbf}}= (\Rep, \Qry)$ be the garbled Bloom filter with hash functions $H_j(x)=F_K(\langle j,x \rangle)$ and $\privaux=langle K \rangle$.  For any set~$S$, let $\leak(S)=|S|$.  There exists a simulator~$\Sim$ and an adversary~$B$ such that, for any adversary~$A$
\[
\AdvPrivSS{\setprim_{\mathrm{ds}},\leak}{A,\Sim} \leq  \AdvPRF{F}{B}
\]
%(Both~$B$ and~$\Sim$ are explicitly constructed in the proof of this theorem.)
...
\begin{proof}
It suffices to give the decomposition of $\Rep$ in to $\Rep_1$ and $\Rep_2$ as required by Theorem~\ref{thm:xxx}.
On input a set of distinct elements \fixme{ANIMESH: make this correct}
$S=\{x_1,x_2,\ldots,x_n\}$, algorithm~$\Rep_1$ returns $s_1=\langle 1,x_1 \rangle, s_2=\langle 2,x_1 \rangle, s_3 = \langle 1,x_2\rangle, s_4=\langle 2,x_2 \rangle,\ldots,s_{2n-1}=\langle 1,x_n \rangle, s_{2n}=\langle 2,x_n \rangle$.

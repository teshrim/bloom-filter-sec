...
data structure.  Let $\leak(S)=|S|$. Then
\[
...
{k}, j \in [n]}$. Let $\Rep_2$ be the algorithm that, on input
$n,y_1, \ldots, y_{\ell(n)}$, initializes an $m$-bit array $M$ to~0
and then sets $M[y_i] \gets 1$ for $i \in [\ell(n)]$. One can easily
verify that $(\Rep_1, \Rep_2, \ell, F)$ is a factoring of~$\Pi$.
Applying Theorem~\ref{thm:priv-ss} yields the claimed result.
\hfill\qed
\end{proof}
...
The domain-separated PRF-hash requires~$k$ PRF evaluations per
set-membership query, and this may be unattractive in certain use
cases.  Kirsch and Mitzenmacher~\cite{kirsch2006less} have shown
that two PRF evaluations suffice, defining the hash functions to be
$h_i(x)=F_K(\langle 1,x \rangle) + j\cdot F_K(\langle 2,x \rangle)
\bmod m$, where~$m$ is the bit-length of the Bloom
...
to security.} We call this the \emph{linear PRF-hash} Bloom filter.
\jnote{Maybe call it ``Pairwise-independent hash''?} In this
section, we show that this scheme is both correct and private.
...
Taken together, the results of this section and the next highlight
the privacy of the linear PRF-hash construction can be sensitive to
its usage.
...
\heading{Privacy. } Consider the set-membership data structure given
in Figure~\ref{fig:lin-and-bi-lin} (right side), which we call the
...
efficient SS-privacy attack even when simulator is provided nearly
the entire set.  Consider the following ``all-but-last'' leakage
function. On input a set~$S=\{x_1,x_2,\ldots,x_n\}$, it assigns
$x'_n \gets x_n[1:|x_n|-1]$ and returns

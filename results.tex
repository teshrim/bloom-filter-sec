...
Fix $n,k,m \geq 0$ and let $\mathcal{S}=[\univ]^n$.  Then $\Pi_{\mathrm{lin}}= (\calQ,\Rep, \Qry)$ is defined as in Figure~\ref{fig:lin-and-ds} (left side).  The following result shows, informally, that if~$F$ is a good PRF, then $\Pi_\mathrm{lin}$ is correct against adaptive error-finding adversaries.
...
\todo{Privacy theorem for the ``plain'' linear-PRF scheme.  Follows from Jon's general theorem.}
...
\todo{Explain why classical BF in the ROM does not meet our SS-privacy notion, and why this is intuitive/supports our definition.}

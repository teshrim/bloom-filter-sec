...
$\pubaux \gets M$; $\privaux \gets $???\\

\medskip
\algorithmv{$\Qry(\pubaux,\privaux,\qry_x)$}\\
$M \gets \pubaux$, ??? $\gets \privaux$\\
$x' \gets 0^\lambda$\\
for $j \in \{1,2,\dots,k\}$\\
\nudge $h_j \gets H_{K_j}(x)$\\
...
\caption{ $\Rep$ and $\Qry$ algorithms for garbled Bloom
  filter $\Pi_{\mathrm{garbled}}$ in the ROM for a hash function $H\colon \calK \times \mathbb{N} \times 
...
{
\adversaryv{$A^{\TestOracle,H}(\pubaux)$}\\
$M \gets \pubaux$\\
for $1 \leq i_1 < i_2 < \cdots < i_k \leq m$\\
\nudge $x \gets M[i_1] \xor M[i_2] \xor \cdots \xor M[i_k]$\\

...
\end{theorem}
\fixme{This result is pretty intutitive following the classic BF correctness result. Probably fine to just explain it and push an appendix, or even drop from camera ready if out of space.}
We note that Kirsch and Mitzenmacher~\cite{kirsch2006less} show
...
\medskip
\algorithmv{$\Qry(\pubaux=M,\privaux=K,q_x)$}\\
%$M\gets\pubaux$, $K \gets \privaux$\\
%\jnote{Fix}\tsnote{Fix what?} \\
for $j \in \{1,2,\ldots,k\}$\\
\nudge $h_j \gets F_{K}(\langle 1,x \rangle) + j\cdot F_{K}(\langle 2,x\rangle) \bmod m$\\
...
The scheme from Niedermeyer et al.\cite{xxx}, $\setprim_{\bilin}$.
...
\jnote{I would emphasize that $F_K(\cdot)$ is computed only twice
per inner loop (which is the whole point of this scheme) by moving
that computation outside the loop.}} \label{fig:lin-and-bi-lin}
...
\rceil$.\tsnote{This value of~$k$ is from the classical analysis for
...
\nudge\nudge $\overline{M}[i] \gets 1$\\
if $\overline{M} \in \pubaux$ then Return 1 \\
Return 0} \caption{Bigram domain-separated PRF-hash.}
\label{fig:DS-bigram}
\end{figure}
...
\ln(2) \alpha \rceil$.\tsnote{This value of~$k$ is from the
...
attacker $B$ \jnote{How do we describe the number of PRF queries $B$
makes? Same problem as in Theorem~\ref{thm:priv-ss}.} for which
...
the \emph{only} information leaked by the data structure is the
sizes of the various intersections of the~$\{B_x\}$ sets.
...
\begin{theorem}[{\rm Correctness of private-$M$ GBF in the ROM}]\fixme{FINISH! (Animesh?)}\label{thm:gbf-correctness}

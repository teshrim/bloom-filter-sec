...
$(1-e^{-kn/m})^k+O(1/n)$. %, although the asymptotic rate of
%convergence is faster than~$1/n$.
...
(q_T+q_H)^r$ (with reasonable tightness when~$r$ is small), in which
case the bound becomes
\[\left((q_T+q_H) \cdot (1-e^{-kn/m})^k + O((q_T+q_H)/n) \right)^r;\]
loosely, and ignoring the low-order term, this is the bound one
expects if we view the attacker as trying to find an error in each
of~$r$ independent``rounds,'' each round consisting of~$q_T+q_H$
attempts. \jnote{But if the attacker makes $q=q_T+q_H$ queries
overall, isn't it only making $q/r$ queries per round?}
%JK

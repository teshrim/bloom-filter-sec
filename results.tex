...
\hfpagess{.45}{.45}
{
\adversaryv{$A^{\TestOracle,H}(\pubaux)$}\\[1ex]
$M \gets \pubaux$; $L \gets \binom{m}{k}$
for $\ell=1$ to $L$\\
$i_1,i_2,\ldots,i_k \gets \mathcal{I}_L$\\
...
{
\adversaryv{$A^{H}(\pubaux)$}\\[1ex]
$M \gets \pubaux$
for $x \in X_1,X_2,\ldots,X_q$\\
\nudge for $j=1$ to $k$ \\
\nudge \nudge $i_j \gets H(j,x)$
\nudge $x' \gets M[i_1] \xor M[i_2] \xor \cdots \xor M[i_k]$\\
...
\caption{Correctness attack for garbled Bloom filters with public~$M$.  The sequence $\mathcal{I}_1,\mathcal{I}_2,\ldots,\mathcal{I}_L$ is some implicit enumeration of~$k$-tuples of indices over $[m]$.   }
\label{fig:adv-gbf}
\end{figure}
...
\heading{Privacy. }
We observe immediately that when~$\pubaux=\langle M \rangle$ the GBF cannot be SS-private (in the ROM) because $\privaux=\emptystring$.  In fact, it is easy to see that the correctness attack given in Figure~\ref{fig:adv-gbf} works as a one-wayness attack, too, with small modifications: remove the check for $x \not\in S$, and the call to $\TestOracle$.  To maximize the efficiency of the attack

 Thus the efficacy of the attack depends on the false positive rate. \todo{Either work out a proof for this case, or punt and make the attack more formal.}
...
\begin{theorem}\label{thm:gbf-prf-privacy}
...
\begin{proof}
It suffices to give the decomposition of $\Rep$ in to $\Rep_1$ and $\Rep_2$ as required by Theorem~\ref{thm:xxx}.
On input a set of distinct elements $S=\{x_1,x_2,\ldots,x_n\}$, algorithm~$\Rep_1$ returns $s_1=\langle 1,x_1 \rangle, s_2=\langle 2,x_1 \rangle, \ldots, s_k\langle k,x_1\rangle, s_{k+1}=\langle 1,x_2\rangle, \ldots, s_{2k}=\langle k,x_2\rangle, \ldots ,s_{kn+1}=\langle 1,x_n\rangle, \ldots, s_{kn}=\langle k,x_n \rangle$.  On input $n,F_K(s_1),\ldots,F_K(s_{kn})$, algorithm $\Rep_2$ carries out the pseudocode of~$\Rep$ in Figure~\ref{fig:gbf}, making use of $F_K(s_1),\ldots,F_K(s_{kn})$ in the obvious way.  It returns~$M$ as $\pubaux$.
\end{proof}

...
\begin{theorem}\fixme{FINISH! (Animesh?)}
...
\[
\AdvPrivOW{\setprim_{\mathrm{gbf}},\distr{}{}}{A_{\mathrm{ow}}} \geq 1 - \left(1-\frac{1}{2^{\mu}} \right)^{nq} \geq 1 - e^{-nq/2^\mu}
\]
 where $\mu = H_\infty(\distr{}{})$ is the pointwise min-entropy of~$\distr{}{}$, and $A_{\mathrm{ow}}$ makes $kq$ random-oracle queries.
\end{theorem}

\todo{If time:  would be great to have an upperbound on onewayness for this construction!}

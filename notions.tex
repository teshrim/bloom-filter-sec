...
\tsnote{Is this the right kind of indistinguishability anyway?  It seems to me that we want to say that a representation~$M$ and the public info~$\aux$ tell you nothing (non-trivial) about the set being represented.  One way to capture this would be indistinguishability with respect to the query set~$\calQ$: you are handed $(M,\aux)$ and you need to output a query~$q\in\calQ$ such that $q(S_0) \neq q(S_1)$.  But this doesn't really capture that $(M,\aux)$ on their \emph{own} do not leak information about the set.  (Classical BFs, by the way, essentially leak the number of elements in the set by looking at the number of 0-bits in the array.)  I don't know, this is tricky... How do you handle the fact that, under our current syntax, $M$~may contain secrets, e.g. a PRP key?}

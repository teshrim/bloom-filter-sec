...
\jnote{Need to relate running time of $\Sim$ to running time of~$A$. Also, the definition
...
Then $\AdvPrivOW{\Pi,\mathcal{D}}{t} \leq \tilde H_\infty({\cal D} \mid {\sf leak}) + \epsilon$.
\end{theorem}
\begin{proof}
...
we must have $\ExpPrivSSsim{\Pi, {\sf leak}}{A', \Sim} \leq H_\infty({\cal D} \mid {\sf leak})$.
This implies
$\delta \leq \tilde H_\infty({\cal D} \mid {\sf leak})+\epsilon$ as desired.
\end{proof}

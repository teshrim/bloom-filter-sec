...
$A'$ running in about the same time for which
\[\AdvPrivSS{\setprim,{\sf leak}}{A} \leq {\sf Adv}_F^{\rm PRF}(A'). \]
\end{theorem}
\begin{proof}
...
outputs~1 in
the following experiment~$G0$: \jnote{BTW, in class when I do this stuff I usually
...
$G0$ is bounded by ${\sf Adv}_F^{\rm PRF}(A')$, where $A'$ performs the
obvious simulation.

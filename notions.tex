...
random oracle. \fixme{I think we should *not* allow $S$ to depend on
  the RO.  That's like letting $S$ depend on $\pub$ in the standard
  model, but $\pub$ hasn't been fixed yet.  } \jnote{I disagree. Plus if you do that, then
  you put the simulator at an unfair disadvantage. (Consider a scheme that always appends
  $H(0)$ to $\pubaux$, for every~$S$. The simulator will never be able to simulate that---even
  if you give it~$S$!---simply because it can't guess~$H(0)$).}
...
\jnote{Need notation for the advantage in the PRF
  experiment.}\tsnote{Currently typeset in an appendix}
...
$B$ \jnote{How do we describe the number of PRF queries $B$ makes? Note that it makes
exactly $t$ queries, but $t$ depends on $|S|$ in a non-specified manner.} for which

...
%attack.
%
On the right side of the figure, we give a definition
inspired by the semantic security notion for
encryption schemes.  We refer to it as semantic-security (of sets)
...
\tsnote{Is this the right kind of indistinguishability anyway?  This
  current notion says, in effect, that $S_0$ and $S_1$ are
  indistinguishable \emph{with respect to the allowed queries}
  (assuming that $\pubaux$ doesn't stupidly leak information.)  But
  why not try to formalize the intution that a representation~$M$ and
  the public info~$\pubaux$ tell you nothing (non-trivial) about the
  set being represented, period?  (Classical BFs, by the way,
  essentially leak the number of elements in the set by looking at the
  number of 0-bits in the array.)   That's tricky to define... I'll
  take a shot at it.}

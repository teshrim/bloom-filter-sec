...
adversarial) notion of correctness. \jnote{I would call it ``soundness'' rather than ``correctness.'' But if you disagree,
just remove this comment and we'll go with correctness.}
...
\jnote{Entirely general, only specializing to set-membership data structures when needed.}
...
In the experiment we track the time complexity (relative to some
...
attacker~$A$, there is an attacker $A'$ running in about the same time with %JK

...
correctness, at least for the schemes we consdier.} \jnote{If instead we choose $S$ from ${\cal D}$, then
...
data structures, however.}\jnote{Why does $A$ output
anything at all?}\tsnote{To signify it is finished execution.  Matter
of taste, I'm not hardover about it.}
...
counter and returned this was to avoid~$A$ be forced to evalutate
queries of unknown computational cost.} 
%\jnote{I also changedthe definition so the adversary has to find $r$
% {\bf unique} inputs on which there is an error.}
...
\jnote{Note~$q$ is overloaded here.}\tsnote{True, and do we even need
  ...
  NY?  Why not just have explict advantage bounding statements, as we
...
introducing the resource-parameterized notation; likewise the others.}
\end{theorem}
\begin{proof}
...
\jnote{OK, from here on I specialize to set-representation data
  structures\ldots} \tsnote{I don't understand this comment.  That's
  what our syntax supports anyway, so how is this specializing?}

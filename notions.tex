...
q(S_1)$. \tsnote{I'm not convinced this is the right restriction.
  ...
  against set membership queries. Do you mean to
  assume that the attacker does not ask~$q$ such that $q(S_0)=q(S_1)$
  because these are pointless?  Otherwise, I don't see a need for restrictions. Maybe we restrict from queries~$q$ that would have the same value for all sets~$S \in \calS$?  (Or all sets of the same size, or...?)}

\tsnote{Is this the right kind of indistinguishability anyway?  It seems to me that we want to say that a representation~$M$ and the public info~$\aux$ tell you nothing (non-trivial) about the set being represented.  To me, this game would be more like, you are handed $(M,\aux)$ and you need to output a query~$q$ such that $q(S_0) \neq q(S_1)$... I don't know, this is tricky.  How do you handle the fact that, under our current syntax, $M$~may contain secrets, e.g. a PRP key?}

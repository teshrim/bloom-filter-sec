...
sets. \jnote{I got rid of the weaker definition, since it seems
  unnatural to me.}\tsnote{Why? It captures things like the Squid
  summary cache -- hash functions are public (in the code, in the
  literature), but you would need to corrupt the HTTP proxy-server to
  get the representation.} \jnote{We can include the weaker definition, or perhaps
  it is cleaner to just have the syntax output $(\pub, \privaux)$ with $\pub$ always
  public and $\privaux$ always private; a scheme can then choose whether $M$ is in $\pub$
  or $\privaux$. If we do this, then need to be careful how to define size.}
...
The latter seems cleaner, so I took that approach here.}\tsnote{I
guess I prefer the former.  Security bounds will always surface the (elementwise)
min-entropy of~$\distr{}{}$.  In any case, it's not clear to me that
the null experiment captures reference/trivial attacks, which is what
I think you are trying to capture with it.  I mean, why not give the
public information? }
...
by a simulator $\calS$ that is only given ${\sf leak}(S)$. \jnote{${\sf leak}$ can
be randomized.} \jnote{Note
...
answer in the simulated world, then this definition seems to imply
correctness also.}

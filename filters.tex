...
\hpagess{.6}{.35}
{
\experimentv{$\ExpFPSecHash{B,\ell}{\distr{U}{n}, A}$}\\
$S_1,S_2,\ldots,S_\ell \getsr \distr{U}{n}$\\
$ \{h_1,h_2,\ldots,h_k\} \getsr \Hash$\\
$\mathcal{C} \gets \emptyset$\\
for $i=1$ to $\ell$\\
\nudge $M_i \gets \Rep^{\HashOracle}(S_i)$\\
$(i,x) \getsr A^{\QryOracle,\CorruptOracle}()$\\
if $i \not\in \mathcal{C} \wedge \Qry^{\HashOracle}(M_i,x)=1$ then \\
\nudge Ret 1\\
Ret 0
}
%
...
\hpagess{.6}{.35}
{
\experimentv{$\ExpFPPubHashBB{B,\ell}{\distr{U}{n}, A}$}\\
$S_1,S_2,\ldots,S_\ell \getsr \distr{U}{n}$\\
$ \{h_1,h_2,\ldots,h_k\} \getsr \Hash$\\
$\mathcal{C} \gets \emptyset$\\
for $i=1$ to $\ell$\\
...
\hpagess{.6}{.35}
{
\experimentv{$\ExpFPPubHash{B,\ell}{\distr{U}{n}, A}$}\\
$S_1,S_2,\ldots,S_\ell \getsr \distr{U}{n}$\\
$ \{h_1,h_2,\ldots,h_k\} \getsr \Hash$\\
$\mathcal{C} \gets \emptyset$\\
for $i=1$ to $\ell$\\
\nudge $M_i \gets \Rep^{\HashOracle}(S_i)$\\
$(i,x) \getsr A^{\QryOracle,\CorruptOracle}(\{h_1,h_2,\ldots,h_k\})$\\
if $i \not\in \mathcal{C} \wedge \Qry^{\HashOracle}(M_i,x)=1$ then \\
\nudge Ret 1\\
Ret 0
}

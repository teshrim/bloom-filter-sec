...
%
The deterministic query algorithm $\Qry\colon \Sigma^m \times U
\rightarrow \bits$ takes a representation $M$ and an element $x \in
U$ and returns a bit.
%
We assume that both $\Rep$ and $\Qry$ have blackbox access to the
functions $h_1,h_2,\ldots,h_k \in \mathcal{H}$.
...
\in S$, it holds that $\Prob{\Qry(\Rep(S),x)=1}=1$, where the
...
make the hash functions secret.} The traditional notion of soundness
...
$1-(1-\epsilon)^q$ plus negligible? One thing that is nice about the
...
gained by considering this definition.} In this section, we consider

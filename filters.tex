...
\heading{Security in multiple-server settings. }
\begin{figure}
\centering
\fpage{.6}{
\hpagess{.6}{.35}
...
\nudge $M_i \gets \Rep_i^{\HashOracle}(S_i)$\\
$(i,x) \getsr A^{\QryOracle,\CorruptOracle}()$\\
if $i \not\in \mathcal{C} \wedge \Qry_i^{\HashOracle}(M_i,x)=1$ then \\
\nudge Ret 1\\
Ret 0
}
%
...
\nudge $ \{h^i_1,h^i_2,\ldots,h^i_k\} \getsr \Hash_i$\\
\nudge $M_i \gets \Rep_i^{\HashOracle}(S_i)$\\
$(i,x) \getsr A^{\QryOracle,\HashOracle,\CorruptOracle}()$\\
if $i \not\in \mathcal{C} \wedge \Qry_i^{\HashOracle}(M_i,x)=1$ then \\
\nudge Ret 1\\ 
Ret 0
}
...
\nudge $M_i \gets \Rep_i^{\HashOracle}(S_i)$\\
$(i,x) \getsr A^{\QryOracle,\CorruptOracle}(\mathcal{H}_1,\mathcal{H}_2,\ldots,\mathcal{H}_\ell)$\\
if $i \not\in \mathcal{C} \wedge \Qry_i^{\HashOracle}(M_i,x)=1$ then \\
\nudge Ret 1\\
Ret 0
}
...
\caption{False-positive security for an $(n,k,m)$-filter~$B$ in the multiple-server setting.  (Three variations, depending on access to/knowledge of hash functions.) }
\label{fig:fp-filter}
\end{figure}
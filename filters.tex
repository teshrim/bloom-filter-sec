...
\heading{FP under adversarially influenced sets.}
\tsnote{Define FP security when the adversary gets to ``influence'' the elements of~$S$, for some defintion of ``influence''.  Note that if the adversary is allowed enough time to compute $|M|$ preimages, it can specify a set~$S$ of size at most~$|M|$ elements that result in a filter that is all 1s. }

...
Fix sets~${\cal U}, \Gamma$, and $\calQ$, where $\calQ = \{\qry
\colon {\cal U} \to \Gamma\}$. A \emph{data structure} (supporting
queries $\calQ$ over universe ${\cal U}$) is a tuple
$\setprim=(\Rep, \Qry)$ where:
\begin{itemize}
...
For a data structure $\setprim=(\Rep, \Qry)$ supporting queries
${\cal Q}$ over universe ${\cal U}$, let
$\mathrm{Err}(S,\qry,\pubaux,\privaux)$ be  true iff
$\Qry(\pubaux,\privaux, \qry) \neq \qry(S)$.  Then $\Pi$ has
\emph{error probability} $\epsilon$ if
...
\heading{Data structures for set-membership queries.} Our particular
focus will be on data structures that support membership queries.
\jnote{I changed this from multiplicity to membership.}

\begin{definition} \rm
A \emph{set-membership data structure over $\univ$} is a data
structure with ${\cal U} \subseteq 2^\univ$, $\Gamma=\{0,1\}$, and

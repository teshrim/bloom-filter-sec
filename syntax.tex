...
\jnote{Multisets?} of~$\univ$. We write $x \getsr \univ$ to denote
...
\begin{itemize}
\item $\Sigma$ is the \emph{representation alphabet}. \jnote{Is
    there any reason not to simply fix this to $\{0,1\}$?}
\item $\Gamma$ is the set of possible \emph{query answers}.
    \jnote{Do we need this for any reason?}
...
\item $\Qry\colon \Sigma^* \times \bits^* \times \calQ \to
    ...
    \emph{answer} $a \in \Gamma$. \jnote{Should take $\pubaux$
    as input also.}
\end{itemize}
\hfill\dqed
\end{definition}
...
secret keys, but this is unnatural.} All of the data structures that
...
if there's more to say in this section, or if what's here is already
too much.}


\heading{Set-multiplicity Data Structures. } Our particular focus
...
structure with $D=\mathbb{N}$ \jnote{Notation. (Anyway, I don't
think we need an explicit symbol for the range of queries.)} and

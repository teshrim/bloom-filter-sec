...
\noindent The definitions above can be lifted to the random-oracle
...
\def\bin{{\sf Bin}}
\heading{``Trivial'' attacks, and error probability vs.\ error
rate.} \jnote{Not sure this belongs here. I'd rather move any
discussion of ``attacks'' (trivial or not) to later. But if we just
want to highlight the difference between error and error rate we
could do that here.} Let~$\Pi$ be a scheme that is
$(1-\epsilon)$-correct, and consider an attack that asks random
...
$(1-\epsilon)$-correctness of~$\Pi$ guarantees that the probability
...
multiplicity queries, of which set-membership queries are a special
case.
\begin{definition} \rm
...
We illustrate our notation by casting Bloom filters as a particular
example of a set-multiplicity data structure. \jnote{Only handles
sets, not multisets.} Let $\Gamma=\bits$, and fix functions
...
\{1,2,\ldots,m\}\}$. \jnote{Note depends on $m$. It might be easier
(especially since we talk about error) to already let the hash
functions be independent random oracles.} It then computes an
$m$-bit array~$M$ (initialized to 0 everywhere) by setting
$M[h_i(x)]=1$ for all $i\in [k]$ and $x \in S$.  Finally, $\Rep$
returns $\privaux=\emptystring$ and $\pubaux = M$ as the
representation.
%
Algorithm~$\Qry$ is defined so that, on input
$(\pubaux,\privaux,q_x)$, it returns the minimum of the values
$M[h_1(x)],\ldots,M[h_k(x)]$. \jnote{How does $\Qry$ know the hash
functions?}

As a simple extension, consider $\Gamma=\mathbb{N}$ and modify the
basic Bloom filter so that it stores counters, rather than bits, at
...
Wen~\cite{xxx}. \jnote{Details have to be filled in.} This stores
...
\tsnote{Check the correctness of this claim.}

...
functions be independent random oracles.} \tsnote{I don't see the
problem.  $\Rep$ knows~$m$, so any function (whose range admits at
least~$m$ values) can be effectively treated as having range~$[m]$.
But I added a sentence at the end of this paragraph, too.  See if that
helps.} 

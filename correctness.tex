...
Adversary~$A$ is given~$\aux$ and (possibly) $\calS$~as input; it is
...
The adversary \emph{succeeds} if it outputs~$r$ distinct queries $q_1, \ldots, q_r \in \mathcal{Q}$ for which
$\Qry(\aux, M, q_i) \neq q_i(S)$ for all~$i \in [r]$
...
\hpagess{.6}{.35}
{
\experimentv{$\ExpCorrect{\setprim,r}{\distr{\calS}{},A}$}\\
$S \getsr \distr{\calS}{}$\\
$\mathrm{err}\gets 0$\\
$(M,\aux) \getsr \Rep(S)$\\
...
Roughly, we say that a set-representation~$\Pi$ is $(r,\epsilon',t)$-FP-secure if the probability that
any $t$-time attacker succeeds is at most~$\epsilon'$. 
...
\hpagess{.6}{.35}
{
\experimentv{$\ExpCorrect{\setprim,t}{\distr{\calS}{},A}$}\\
$\calS \getsr \distr{U}{n}$\\
$(M,\aux) \getsr \Rep^{\HashOracle}(\calS)$\\
$\calX \getsr A^{\QryOracle}(\calS,\aux)$\\
if $|\calX| < t$ or $\calX \cap \calS \neq \emptyset$ then \\
\nudge Ret 0\\
If $\exists x \in \calX$ s.t. $\Qry(M,\aux,x)=0$ then\\
\nudge Ret 0\\
Ret 1
}
%

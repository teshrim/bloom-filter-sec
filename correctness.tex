...
most~$\epsilon'$.  \tsnote{Maybe not... revisit.}


\def\bin{{\sf Bin}}
\heading{Defining adversarial advantage. } 
For any scheme~$\Pi$ there is a trivial,
\emph{non-adaptive} attack against this that wins a noticible fraction of the time.  In
particular, let $q_1,q_2,\ldots,q_T$ be uniform samples from~$\calQ$.
...
If~$\Pi$ is $\epsilon$-correct, then the probability that $X_i=1$ is at most~$\epsilon$.

...
Let $D$ be a zero-query adversary(shown in Figure~\ref{fig:lin-correctness-zq-adv}) corresponding to $A$ attacking $\game{3}$; it finds $r$ false positives against $\tilde{\setprim}_{\mathrm{ds}}$. It simulates the~$\Test$ oracle by returning 1 on the guessed queries, and 0 otherwise; and it halts after finding $r$ FP's. Intuitively, it  guesses which~$r$ of~$A$'s queries will be false positives.  So, $\AdvCorrect{\tilde{\setprim}_{\mathrm{ds}},r}{D} = \Prob{\game{3}(A) = 1}$.

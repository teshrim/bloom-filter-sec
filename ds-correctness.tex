...
Theorem~\ref{thm:ds-correctness}.} \label{fig:ds-correctness-games}
\end{figure}
...
\nudge When $A$ halts:\\
\nudge\nudge Return $\bot$ } } \caption{Left: Adversary $B$
simulating $\game{0}(A)$ and $\game{1}(A)$ respectively for the
proof of Theorem~\ref{thm:ds-correctness}. Right: Adversary $D$ for

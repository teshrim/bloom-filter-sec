...
We use a game playing argument to prove this theorem. Please refer to Figure~\ref{fig:ds-correctness-games}. In game \game{0}, adversary, $A$ attacks the correctness game with  hash functions $h_j(x) = \FK $, and hence exactly simulates $\ExpCorrect{\setprim_{\mathrm{ds}},r}{A}$.
\begin{equation}
\AdvCorrect{\setprim_{\mathrm{ds}},r}{A} = \Prob{\game{0}(A)=1}
\end{equation}

In game $\game{1}$, $A$ attacks a different filter $\tilde{\setprim}_\mathrm{ds}$, in which $F_K$ is replaced by a random function. Let $B$ be a prf-adversary as shown in Figure~\ref{fig:ds-correctness-adv}. Based on its challenge bit $b$, it exactly simulates $\game{0}$ or $\game{1}$. So,
\begin{align*}
...
\fixme{Animesh: remove Crefs, fix captions to look like the classical
  BF games captions}
\end{figure}
...
\caption{Left: Adversary $B$ simulating $\game{1}(A)$ and $\game{2}(A)$ respectively for the

...
We note that since game~$\game{5}(A)$ returns iff~$A$ manages to find~$r$ errors the game managed to guess exactly which $\HashOracle$ queries would cause the errors
...
\hpagessl{.45}{.5}
{
\adversaryv{$A_0\TestOracle,\HashOracle(\pubaux)$}\\
$c\gets 0$, $\calY \gets \emptyset$\\
$\calC \gets \emptyset$\\
$\mathcal{I}\getsr [\{1,2,\ldots,q_T + q_H\}]^r$\\
$\bot \getsr A^{\mathsc{TEST},\mathsc{HASH}}(\pubaux)$\\

\oracle{$\mathsc{TEST}(\qry_x)$}\\
if $x \in \mathcal{C}$ then Return $\bot$\\
$\calC \gets \calC \cup x$\\
if $\Ans[x] = \undefined$\\
\nudge $(h_1, \ldots, h_{k}) \gets \HashOracle(\qry_x)$\\

...
\tsnote{Perhaps a more obvious way to capture the idea that ``finding
  some FPs doesn't help find additional ones'' is to parameterize my notion(s) with a number~$n$, and say
that that the adversary wins iff it outputs~$n$ FPs (these may have
been previously queried to the set-membership oracle).  
Note that in the ROM (and probably in practice) finding~$n>1$ FPs will
require~$n$ times the work; finding the first won't help in finding
the second.} 

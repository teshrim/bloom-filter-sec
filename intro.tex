...
First, $M \getsr \mathbf{B}_1(\mathcal{S})$ for some set
$\mathcal{S}$ that can be adversarially chosen. Then the adversary
may make queries $x_1,x_2,\ldots,x_q$ to an oracle for
$\mathbf{B}_2(M,\cdot)$.  At the end of its execution, the adversary
outputs~$x^*$ and wins the game if (1) $x^* \not\in \mathcal{S} \cup
\{x_1,x_2,\ldots,x_q\}$, and (2) $\mathbf{B}_2(M,x^*)=1$.
...
\jnote{I would say that the trivial scheme \emph{is} secure!} The
surrounding text does say that $\mathbf{B}_1$ ``outputs a compressed

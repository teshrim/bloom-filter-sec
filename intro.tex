\jnote{Are we being too specific by focusing on hash-based
set-membership only? It would be nice to give general definitions
that apply to arbitrary data structures (constructed in arbitrary
ways) and then only later specialized to hash-based Bloom filters.}
...
citations}  Given that NY is addressing such a useful real-world
...
First, $M \getsr \mathbf{B}_1(\mathcal{S})$. \jnote{Where does the
set come from? Is it random? Adversarially chosen?}
\tsnote{In their formalism, it is implicitly an input to the security experiment.
Security is universally quantified over sets~$\mathcal{S}$.  This
would seem to be equivalent to letting the adversary
pick~$\mathcal{S}$ (prior to the hash functions being picked), with
the time to compute and write out~$\mathcal{S}$ folded into the
adversary's running time. } 
Then the
...
First, we note that the security notion forbids~$A$ to win if it
outputs $x^* \in \{x_1,x_2,\ldots,x_q\}$, even if one of these was a
false positive. 
\jnote{We discussed this on the phone, but let me
put it in writing here. I think this is actually a reasonable choice
because it captures the notion that ``even after finding some false
positives, it remains hard to find additional false positives.''
Note that in practice finding false positives is ``easy'' since
$\epsilon$ might be $1/1000$ rather than negligible.} 
\tsnote{For me the question is: is security violated once the first FP is found, or
the second?  My answer, which is in line with the traditional
soundness notion, is that the first FP is a security violation
even if finding it isn't ``hard'' by cryptographic standards.
But I do appreciate your point.  A natural goal is to say that, in some sense,
\emph{adaptivity doesn't make it any easier} to find a FP.  Adapting
to what, though?  In the traditional notion, $x$~is picked before the
coins of~$\mathbf{B}_1$ are flipped, i.e., before the hash functions
are sampled.  The direct non-adaptive generalization is to say that
$x_1,x_2,\ldots,x_q$ are all picked up front.  The direct
\emph{adaptive} generalization would allow $x_1,x_2,\ldots,x_q$ to
depend in the coins of~$\mathbf{B}_1$, i.e., the hash functions.  The
NY definition captures only indirect dependence: adversaries that can adapt to set-membership queries,
but \emph{not} (directly) to the input-output behaviors of the hash
functions.  The three notions I've proposed in Figure 1 cover a
spectrum of adaptivity: the first is adapting to set-membership queries
only, the second to those and hash input-output behavior, the third to
set-membership queries and full knowledge of the hash functions.  Each
has it's own associated (informal) security interpretation.}

\tsnote{Perhaps a more obvious way to capture the idea that ``finding
  some FPs doesn't help find additional ones'' is to parameterize my notion(s) with a number~$f$, and say
that that the adversary wins iff it outputs~$f$ FPs (these may have
been previously queried to the set-membership oracle).  
Note that in the ROM (and probably in practice) finding~$f>1$ FPs will
require~$f$ times the work.  Right?} \tsnote{FYI: this is how MAC security against multiple forgeries is defined.  See \texttt{https://eprint.iacr.org/2006/095.pdf} for details.}
%\tsnote{Here's an off-the-wall idea.  What if the basic FP security
%notion gave the adversary an oracle for set-membership (and the hash functions, depending on the setting), and after interacting with these
%it is challenged with either $x_0$ or $x_1$, where $x_0$ is a random
%point from the set of non-FPs, and $x_1$ is a random point from the
%set of FPs?  (No oracles after being challenged.)  The adversary has
%to decide which it received.}


Second, we note that the NY syntax does not capture the size of~$M$
as a parameter; 
\jnote{I think this is also reasonable; typically
security is defined without regard for efficiency}
\tsnote{My complaint isn't about efficiency. It's about the fact that,
by not including $|M|$ in the definition, the trivial scheme I give
(just below) is allowed, and is secure.  Anyway, as I say at the end
of this paragraph, this is a small(er) complaint than what follows}. The surrounding
...
%$\mathbf{B}_1$ is sampling $h_1,h_2,\ldots,h_k$, it seems that their
%formalization fails to capture the basic Bloom filter construction;
%at best, it is hard to see clearly that it does.

\jnote{FWIW, I think the classical description of Bloom filters has
the same problem. I.e., if the hash functions are not fixed, then
their description should be counted as part of the length of the
data structure. On the other hand, if they are fixed, then the
false-positive rate is not well-defined (since there is no
probability space to speak of). So in that sense, NY just carried
over the problem from the classical literature.}
\tsnote{One can suggest that it \emph{should} be counted in the classical
  description, but the fact is that it isn't :)  And in the 30+ years
  since the original paper, no one saw fit to correct this?  From an
  algorithmic perspective, I think the classical approach makes
  sense.  The questions being answered are: how many bits do I need to
  store a representation of the set for a given FP rate, and how does this compare to the
  combinatorially computed lower-bound?  The description length of the
hash functions may be bigger or smaller, but the number of bits you
need to hold the set representation is independent of that.  (Imagine
that you have long random bitstrings in the set~$\mathcal{S}$.  Then
whether your hash functions simply truncate, or they are random
oracles, the size of the representation as a function of the target FP
rate doesn't change.  Yet the description lengths of the hash
functions are drastically different.)}

On the other hand, assume the hash functions are implicitly
hard-coded into~$\mathbf{B}_1$ (and $\mathbf{B}_2$). Then we can
treat~$\mathbf{B}_1$ as deterministic, since it will simply carry
out the steps required to populate an array using these hash
functions, and output the array~$M$ as the representation.  If the
descriptions of~$\mathbf{B}_1,\mathbf{B}_2$ are known to the
adversary, who is also provided with~$\mathcal{S}$, then the
adversary can compute~$M$ itself, as well as test for false
positives itself. So it isn't clear how the security notion helps to
reason about difficulty of finding a false-positive. Moreover, NY
stresses (at least once) that the representation is \emph{not} given
to the adversary in their notions.  So it seems unlikely that they
intended for $h_1,h_2,\ldots,h_k$ to be hard-coded into the
algorithms. 
\jnote{Perhaps it is reasonable to consider a definition
in which the set is not given to the adversary?}
\tsnote{A FP security notion?  Yes, perhaps so.  It's probably a more
  realistic notion anyway -- is a spammer really going to know my
  whitelist a priori?  (If he does, I probably have bigger problems in
my system than receiving new spam email.)  Or set of URLs cached by a
CDN server?}

Continuing on this point, it seems that the coins of~$\mathbf{B}_1$
...
Bloom filters.  Squid, in particular, uses the basic Bloom filter with four hash
...
HBase~\cite{xxx}, \tsnote{make an impressive list.}  

There are applications in which the proposed Bloom filter instantiations do
...
only to the Bloom filter array.\tsnote{This shows that
false-positive security and privacy (whatever it is) are orthogonal,
since I'm pretty sure this ``encrypted'' bloom filter resists FPs.})
...
caches, P2P network routing algorithms, etc. \tsnote{Could also
point out that their positive results use constructions that have
secret keys, and these are unlikely to be of any use in major
applications like CDNs or P2P networks.  But maybe the horse is
sufficiently beaten by now...}
...
\jnote{I find ``mutable'' a slightly awkward term,  maybe because it
makes me think of mutability in the context of programming
languages. In much data structures literature, this is called the
``dynamic'' case, in contrast to the ``static'' case.} \tsnote{I
adopted this term after reading it in some bloom filter
paper(s). Ironically, I liked it for the same reason you think it is
awkward :)  My PL association to the term is that an immutable object
is one whose state cannot be modified after it is created, and a
mutable object is the opposite.  But I don't really care too much.}, in the sense
...
\item Naor and Yogev
\item Eujin Goh, ``Secure Indexes'' \tsnote{A secure index can be used for set membership.  Builds a secret-key data structure (an Index) that allows searching for keyword~$w$ if one holds the trapdoor $T_w$ for~$w$, where the trapdoor depends on the secret key.  Main construction uses traditional Bloom filters and a PRF.  Construction appears quite inefficient, needing a very long secret key, turning a keyword~$w$ into a bunch of PRF outputs, and then storing each of these PRF outputs in the BF.  Havent read the full analysis; don't know if this was ever published. }
\end{itemize}

...
\jnote{I like soundness better than correctness, but I won't
insist.} and \emph{correctness} are considered. In this work we
...
\todo{highlight that we are able surface ways in which constructions
...
\todo{Somewhere in the intro, might what to face the ``nothing hard''
  ...
  connnecting the adaptive setting to the zero-query setting.}
...
\item Naor and Yogev analyze a new Bloom-filter construction of
    ...
    \jnote{Do we? Perhaps we want to explicitly adapt some of
    our definitions to the ROM?}
\end{itemize}
...
We give a more detailed comparison to, and criticism of~\cite{naor2015bloom} in Appendix~\ref{sec:xxx}.
...
\todo{Surface contribution of setting the stage for a bunch of future
  ...
  hierachical BFs, count-min-sketches, etc.); security of common
  usages of BF and related data structrues (e.g. CDNs and other
